...
export default function WallDecorCategory() {
  const { slug } = useParams();
  const subcategory = wallDecorSubcategories.find((s) => s.slug === slug);
  ...
  if (!subcategory) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-24 text-center">
        ...
        <Link to="/shop/wall-decor" className="text-blushDark underline">
          Back to Wall Décor
        </Link>
      </div>
    );
  ...
  const products = getWallDecorProducts(subcategory);
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-16">
      ...
      <Link to="/shop/wall-decor" className="text-blushDark underline mt-10 inline-block">
        ← Back to Wall Décor
      </Link>
    </div>
  );
