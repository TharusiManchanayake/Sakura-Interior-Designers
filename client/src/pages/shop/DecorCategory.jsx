...
import { decorSubcategories, getDecorProducts } from "../../data/shopData";

export default function DecorCategory() {
  const { slug } = useParams();
  const subcategory = decorSubcategories.find((s) => s.slug === slug);

  if (!subcategory) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-24 text-center">
        <p className="text-2xl">Category not found.</p>
        <Link to="/shop/decor" className="text-blushDark underline">
          Back to Décor
        </Link>
      </div>
    );
  ...
  const products = getDecorProducts(subcategory);
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-16">
      ...
      <Link to="/shop/decor" className="text-blushDark underline mt-10 inline-block">
        ← Back to Décor
      </Link>
    </div>
  );
