...
import { accessoriesSubcategories, getAccessoriesProducts } from "../../data/shopData";
import { useCart } from "../../context/CartContext";

export default function AccessoriesCategory() {
  const { slug } = useParams();
  const subcategory = accessoriesSubcategories.find((s) => s.slug === slug);
  ...
  if (!subcategory) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-24 text-center">
        ...
        <Link to="/shop/accessories" className="text-blushDark underline">
          Back to Accessories
        </Link>
      </div>
    );
  ...
  const products = getAccessoriesProducts(subcategory);
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-16">
      ...
      <Link to="/shop/accessories" className="text-blushDark underline mt-10 inline-block">
        ← Back to Accessories
      </Link>
    </div>
  );
